...
class DBphone(context: Context) : SQLiteOpenHelper(context, "number.db", null,1) {

    override fun onCreate(db: SQLiteDatabase?) {
        var sql = "create table PhoneTable( " +
                "idx integer primary key autoincrement, " +
                "nameData text not null, " +
                "numberData text not null, " +
                "relateData text not null " +
                ")"
        db?.execSQL(sql)
